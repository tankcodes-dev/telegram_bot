...
async function checkStore2() {
	...
	try {
		...
		if(response.status !== 200){
			storeResponse.msg = "Error in Request";
			storeResponse.inStock = false;
			return;
		}
		...
		if (inventory_quantity > inventory_low_stock_quantity) {
			...
			await sendMessage(
				"Amul Chocolate Whey Protein is now in stock. Hurry Up!"
			);
			return storeResponse;
		} else {
			storeResponse.msg = "Chocolate Protein Out of Stock";
			return storeResponse;
		}
	} catch (e) {
...
async function checkStore() {
	try {
		...
		if (inventory_quantity > inventory_low_stock_quantity) {
			...
			storeResponse.msg = "Whey protein Out of Stock";
			return storeResponse;
		}
	} catch (e) {
